...
async function createOrderViaWallet(confirmationToken, paymentMethodId) {
  ...
  const orderData = {
    pageId: "gNgfa61tgV7dAMRSv4NVwiAdoiqVabF0oADvTmqt7rxHg1Nbg-t9zaTRG4b1hsDn",
    action: "process",
...
const getStates = async (countryIso2Code) => {
  ...
  try {
    if (!countriesStatesCache) {
      const res = await fetch('/69b96874a6252bab6d955c39-preview/scripts/countriesData/countriesStates.json', { cache: 'no-store' });
